...
def find_available_moves(board, taboolist):
    ...
    if len(one_value) != 0:
        ...
        for move in one_value:
            newboard.put(*move)
        exist=find_available_moves(newboard, taboolist)
        return one_value + exist[0],exist[1]
...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        fake_moves = set(all_moves(board, taboo)).difference(set(satisfy[0]))
        fake_moves=fake_moves.difference(set(satisfy[1]))
        ...
        if len(satisfy[0])==0:
            self.propose_move(
                        Move(satisfy[1][0][0], satisfy[1][0][1], satisfy[1][0][2]))
        else:

            self.propose_move(
                        Move(satisfy[0][0][0], satisfy[0][0][1], satisfy[0][0][2]))
